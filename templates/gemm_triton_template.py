...
Safety notes: Wave64 compatible, auto-tuned block sizes, split-K support
...
def launch_gemm(A, B, C, M: int, N: int, K: int):
    """Launch the Triton GEMM kernel with auto-grid."""
    BLOCK = {{ TILE_SIZE }}
    grid = (
        (M + BLOCK - 1) // BLOCK,
        (N + BLOCK - 1) // BLOCK,
